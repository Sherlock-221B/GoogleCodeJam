...
struct dir
{
    vector<pair<string, struct dir *>> subdir;
};
...
struct dir *find_dir(vector<pair<string, struct dir *>> &subdir, string dir)
{
    for (int i = 0; i < subdir.size(); i++)
    {
        if (subdir[i].first == dir)
            return subdir[i].second;
    }
    return NULL;
}

int process(struct dir *d, vector<string> &v)
{
    int count = 0;
    for (int i = 0; i < v.size(); i++)
    {
        struct dir *child = find_dir(d->subdir, v[i]);
        if (child != NULL)
        {
            d = child;
            cout << "Exist : " << v[i] <<" & inside ="<<endl;
            for(int j=0;j<child->subdir.size();j++){
                cout<<" "<<(child->subdir[j]).first; 
            }
            cout<<"---"<<endl;
        }
        else
        {
            ...
            d->subdir.push_back(make_pair(v[i], child));
            count++;
            cout << "Creating : " << v[i] << " & counting : " << count << " & " << child << endl;
        }
    }
    return count;
}
